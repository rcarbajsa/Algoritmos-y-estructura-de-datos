...
public class Punto {
  ...
  public boolean accesible(PuntoCardinal direccion) {
    switch (direccion) {
    case NORTE: return norte;
    case SUR: return sur;
    case ESTE: return este;
  ...
  public static void printPuntos(Punto[][] puntos, int currY, int currX)
  {
    printPuntos(puntos,currY,currX,true);
  }

  public static void printPuntos(Punto[][] puntos)
  {
    printPuntos(puntos,0,0,false);
  }
  
  public static void printPuntos(Punto[][] puntos, int currY, int currX, boolean hasCurrPos)
  {
    ...
    for(int i = puntos.length-1; i >= 0; i--) {
      for(int j = 0; j < puntos[i].length; j++)
        System.out.print(printNorthRoad(puntos,i,j,maxY,maxX));
      System.out.println();
      
      for(int j = 0; j < puntos[i].length; j++) {
        System.out.print(printWestRoad(puntos,i,j,maxY,maxX));
        String contents = " ";
        ...
        System.out.print(printEastRoad(puntos,i,j,maxY,maxX));
      }
      System.out.println();
      
      for(int j = 0; j < puntos[i].length; j++)
        System.out.print(printSouthRoad(puntos,i,j,maxY,maxX));
      if (i==0) {
        System.out.println();
        for(int j = 0; j < puntos[i].length; j++)
          System.out.print(printXcoord(j));
      }
    }
    System.out.println();
  }
